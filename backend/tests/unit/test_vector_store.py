...
class TestArticleMetadata:
    ...
    def test_add_article_with_people(self, test_vector_store):
        ...
        retrieved_people = test_vector_store.get_people_from_article("AI Technology Article")
...
class TestSearch:
    ...
    def test_search_with_article_filter(self, test_vector_store):
        ...
        chunks1 = [ArticleChunk(article_title="Article About AI", chunk_index=0, content="AI technology")]
        chunks2 = [ArticleChunk(article_title="Article About ML", chunk_index=0, content="ML algorithms")]
        ...
        results = test_vector_store.search(
            query="technology",
            article_title="Article About AI"
        )
...
class TestPeopleManagement:
    ...
    def test_get_people_from_article(self, test_vector_store):
        ...
        people = [
            Person(nombre="Alice", cargo="CEO", organizacion="TechCorp", datos_interes="Founder"),
            Person(nombre="Bob", cargo="CTO", organizacion="TechCorp", datos_interes="Engineer")
        ]
    ...
    def test_find_articles_by_person(self, test_vector_store):
        ...
        person1 = Person(nombre="Dr. Smith", cargo="Researcher", organizacion="", datos_interes="")
        person2 = Person(nombre="Dr. Jones", cargo="Professor", organizacion="", datos_interes="")

        article1 = Article(title="Research Paper", content="", article_link="url1", people=[person1])
        article2 = Article(title="Conference Talk", content="", article_link="url2", people=[person1])
        article3 = Article(title="Book Review", content="", article_link="url3", people=[person2])
    ...
    def test_find_articles_by_person_case_insensitive(self, test_vector_store):
        ...
        person = Person(nombre="John Doe", cargo="Developer", organizacion="", datos_interes="")
        article = Article(title="Tech Article", content="", article_link="", people=[person])
    ...
    def test_find_people_by_role(self, test_vector_store):
        ...
        people1 = [
            Person(nombre="Alice", cargo="CEO", organizacion="CompanyA", datos_interes=""),
            Person(nombre="Bob", cargo="Developer", organizacion="CompanyA", datos_interes="")
        ]
        people2 = [
            Person(nombre="Charlie", cargo="CEO", organizacion="CompanyB", datos_interes="")
        ]

        article1 = Article(title="Article 1", content="", article_link="", people=people1)
        article2 = Article(title="Article 2", content="", article_link="", people=people2)
    ...
    def test_get_all_people_with_frequency(self, test_vector_store):
        ...
        person_alice = Person(nombre="Alice", cargo="CEO", organizacion="", datos_interes="")
        person_bob = Person(nombre="Bob", cargo="CTO", organizacion="", datos_interes="")

        # Alice appears in 2 articles, Bob in 1
        article1 = Article(title="A1", content="", article_link="l1", people=[person_alice])
        article2 = Article(title="A2", content="", article_link="l2", people=[person_alice, person_bob])
