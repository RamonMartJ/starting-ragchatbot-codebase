...
def test_article_search_tool_with_filter():
    ...
    if len(titles) > 0:
        ...
        try:
            ...
            pytest.fail(f"ArticleSearchTool.execute() with filter raised exception: {e}")
    else:
...
def test_tool_manager_can_execute_tools():
    ...
    try:
        ...
        pytest.fail(f"ToolManager.execute_tool() for people search raised exception: {e}")
...
def test_tools_track_sources():
    ...
    assert hasattr(article_tool, 'last_sources'), "ArticleSearchTool missing last_sources attribute"
    ...
    assert hasattr(people_tool, 'last_sources'), "PeopleSearchTool missing last_sources attribute"
